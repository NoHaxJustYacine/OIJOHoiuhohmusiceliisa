...
const command = args.shift().toLowerCase();
    ...
    if(cmd === 'play') {
      ...
      voiceChannel.join()
      if(!args[0]) return msg.channel.send(`**> Y-MUSIC v1.6 Music Bot:
>    -play \`\`<song name>\`\`
>    -vol \`\`<volume>\`\`
>    -skip**`)
...
// 2-0-0-2
    ...
    if (message.content.startsWith(prefix + 'setStreaming')) {
      ...
      message.delete();
      client.user.setGame(argresult, 'https://twitch.tv/Yacine');
...
Y-MUSIC BOT v1.6 - CODE BY : Yacine
- انشاء الله يفيدك البوت
- #rep 443461392994140170
**`
...
client.login(process.env.BOT_TOKEN).catch(err=> console.log("قد لايوجد توكن او التوكن متوقف . [Yacv1.6]"));